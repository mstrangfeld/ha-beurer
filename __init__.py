...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    LOGGER.debug(f"Setting up Beurer for {address}")

    device = bluetooth.async_ble_device_from_address(hass, address.upper(), connectable=True)
    if device is None:
        raise ConfigEntryNotReady(
            f"Could not find Beurer device at {address}; ensure it is in range and powered on"
        )

    instance = BeurerInstance(hass, address)
