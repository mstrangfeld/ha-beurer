...
async def get_device(mac: str) -> Optional[BLEDevice]:
    devices = await BleakScanner.discover()
...
class BeurerInstance:
    def __init__(self, hass: HomeAssistant, mac: str) -> None:
        ...
        # Serializes connect attempts so a second call doesn't fire while
        # the first is still in flight — that was the source of the
        # "InProgress" / "br-connection-canceled" cascade in stock bleak.
        self._connect_lock = asyncio.Lock()
        self._trigger_update: Optional[Callable] = None
        self._is_on = False
        self._light_on = None
        self._color_on = None
    ...
    async def _do_connect(self) -> bool:
        device = bluetooth.async_ble_device_from_address(self._hass, self._mac.upper(), connectable=True)
        if device is None:
            LOGGER.warning(f"Beurer device {self._mac} is not currently advertising")
            return False
        try:
            ...
            if not self._read_uuid or not self._write_uuid:
                LOGGER.error("No supported read/write UUIDs found on Beurer device")
                await self.disconnect()
                return False

            LOGGER.info(f"Read UUID: {self._read_uuid}, Write UUID: {self._write_uuid}")
            await self._client.start_notify(self._read_uuid, self.notification_handler)
            await self.triggerStatus()
            return True
        except BLEAK_RETRY_EXCEPTIONS as error:
            LOGGER.error(f"Error connecting to {self._mac}: {error}")
            await self.disconnect()
            return False

    async def _write(self, data: bytearray) -> None:
        if not await self._ensure_connected():
            LOGGER.warning(f"Skipping write to {self._mac}: not connected")
            return
        try:
            LOGGER.debug("Sending in write: " + ''.join(format(x, ' 03x') for x in data) + f" to characteristic {self._write_uuid}")
            await self._client.write_gatt_char(self._write_uuid, data)
        except BleakError as error:
            LOGGER.warning(f"Error while trying to write to device: {error}")
            await self.disconnect()
    ...
    async def sendPacket(self, message: list[int]) -> None:
        length = len(message)
    ...
    async def triggerStatus(self) -> None:
        await self.sendPacket([0x30, 0x01])
        await asyncio.sleep(0.2)
        await self.sendPacket([0x30, 0x02])
        LOGGER.debug("Triggered update")

    async def trigger_entity_update(self) -> None:
        if self._trigger_update:
            LOGGER.debug(f"Triggering async update")
            self._trigger_update()
        else:
            LOGGER.debug(f"No async update function provided yet: {self._trigger_update}")

    async def notification_handler(self, characteristic: BleakGATTCharacteristic, res: bytearray) -> None:
        """Handle status notifications from the lamp."""
    ...
    async def update(self) -> None:
        if not await self._ensure_connected():
            LOGGER.info("Was not able to connect to Beurer for update")
            return
        try:
            await self.triggerStatus()
        except BleakError as error:
    ...
    async def disconnect(self) -> None:
        ...
        if client is not None and client.is_connected:
            try:
                ...
                LOGGER.debug(f"Error during disconnect: {error}")
        self._is_on = False
        self._light_on = False
        self._color_on = False
        self._write_uuid = None
